ActiveAdmin.register ActiveAdmin::Audit::ContentVersion, as: 'ContentVersion' do
  ...
  filter :whodunnit, input_html: { class: 'chosen' }, as: :select, collection: -> { ActiveAdmin::Audit.configuration.user_class_name.to_s.classify.constantize.all.map { |u| [u.email, u.id] } }
  filter :created_at
